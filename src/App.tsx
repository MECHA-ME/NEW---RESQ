...
export default function App() {
  ...
  // Accurate GPS: high-accuracy watchPosition, throttled on move (max once per 8s), always includes accuracy
  ...
  useEffect(() => {
    ...
    // Use 4-decimal (~11m) for dedup so real movement still pushes; 5-dec was too sensitive to jitter
    const key = `${deviceFix.lat.toFixed(4)},${deviceFix.lng.toFixed(4)}`;
    const now = Date.now();
    if (key === lastLocPostRef.current.key || now - lastLocPostRef.current.at < 8000) return;
    lastLocPostRef.current = { key, at: now };
    fetch(`/api/users/${userId}/location`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ lat: deviceFix.lat, lng: deviceFix.lng, accuracy: deviceFix.accuracy }),
